...
import { Response } from '../src/utils/classes.js'
...
export function parseVehicleLengthFromDimensions(raw) {
  if (!raw) return 0
...
export function loadRequiredWeightKg(load) {
  if (
...
export function loadMaxItemLengthMm(load) {
  let mm = 0,
    topItems = []
  for (const stop of load?.load_stops || []) {
    for (const order of stop?.load_orders || []) {
...
export function isAssmLoad(load) {
  for (const stop of load?.load_stops || []) {
    for (const order of stop?.load_orders || []) {
...
export function zoneKeysForLoad(load) {
  const set = new Set()
...
export function scoreVehicle(v, needKg, needLenMm) {
